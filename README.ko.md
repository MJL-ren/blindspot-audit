...
- 필요할 때는 좁혀서 판다: `focus: ux-ui`와 `focus: security`는 한 도메인 전용
  심층 프로브 팩을 로드하고, 전체 감사는 주인의 약한 도메인 표면(엔지니어 주인의 UI,
...
`focus: security`는 무엇을 보호해야 하는지, 사람이나 시스템이 어느 신뢰 경계를
넘는지, 권한을 실제로 어디서 확인하는지, 비밀정보와 배포물이 어떤 경로로
움직이는지, 문제가 생겼을 때 주인이 어떻게 알아차리고 복구할지를 살펴본다.
현재 파일, Git 이력, 배포 결과물, 서비스 제공자 쪽 자격 증명 상태를 따로 구분해서
한 곳만 정리하고 전체가 해결됐다고 닫지 않게 한다.

이 포커스는 방어적이고 읽기 중심으로 움직인다. 비밀값은 가리고, 공격 요청을
보내거나 자격 증명을 사용하거나 실제 시스템을 찌르는 대신, 허가된 스캐너·시험
환경 확인·전문가 검토를 다음 확인 방법으로 제안한다.
...
`security` 포커스 팩의 감사 구조와 방어 경계는 아래 MIT 오픈소스 프로젝트를
참고했으며, 팩의 문장은 전부 새로 작성했다.

- [cloudflare/security-audit-skill](https://github.com/cloudflare/security-audit-skill)
  - 사전 구조 파악과 신뢰 경계 지도, 중복 발견 묶기, 독립 검증 구조.
- [gitleaks/gitleaks](https://github.com/gitleaks/gitleaks)
  - 현재 파일과 Git 이력의 비밀정보 확인 분리, 기준선, 식별 지문, 좁은 예외 처리.
- [microsoft/agent-governance-toolkit](https://github.com/microsoft/agent-governance-toolkit)
  - 정책 확인 실패 시 차단하는 동작과 승인한 정확한 행동에만 묶이는 승인 설계.

## 보안

스크립트가 하는 일, 네트워크에 닿지 않는 범위, 문제를 비공개로 신고하는
방법은 [SECURITY.md](./SECURITY.md)를 보면 된다.
패키지 helper에는 읽기 중심의 인벤토리·비밀정보 확인, 검증되는 임시 원장 파일,
선택적인 `127.0.0.1` 결정판이 포함된다. 스킬을 설치해도 외부 네트워크나 provider
접근 권한이 생기지는 않는다.
