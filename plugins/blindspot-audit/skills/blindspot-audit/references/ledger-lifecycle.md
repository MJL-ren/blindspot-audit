...
After an explicit owner reply, stop here and load
`references/owner-response-guard.md`. That delayed reference owns the fresh
owner-response snapshot, v1/v2 response schemas, localized `applicationMap`,
preview, security-batch scaffold, final validation, archive checks, and
`cleanup --confirm-applied`. Do not prepare or apply the reply from memory.
